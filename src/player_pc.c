...
{
    u8 i = 0;
    ClearItemSlots(gSaveBlock3Ptr->pcItems, PC_ITEMS_COUNT);
...
void ItemStorage_RefreshListMenu(void)
{
    ...
    {
        CopyItemName_PlayerPC(&sItemStorageMenu->itemNames[i][0], gSaveBlock3Ptr->pcItems[i].itemId);
...
static void ItemStorage_MoveCursor(s32 id, bool8 onInit, struct ListMenu *list)
{
    ...
    if (sItemStorageMenu->toSwapPos == NOT_SWAPPING)
    {
        ...
        if (id != LIST_CANCEL)
            ItemStorage_DrawItemIcon(gSaveBlock3Ptr->pcItems[id].itemId);
        else
...
static void ItemStorage_PrintMenuItem(u8 windowId, u32 id, u8 yOffset)
{
    if (id != LIST_CANCEL)
    {
        ...
        ConvertIntToDecimalStringN(gStringVar1, gSaveBlock3Ptr->pcItems[id].quantity, STR_CONV_MODE_RIGHT_ALIGN, 3);
...
static void ItemStorage_PrintDescription(s32 id)
{
    ...
    if (id != LIST_CANCEL)
        description = (u8 *)ItemId_GetDescription(gSaveBlock3Ptr->pcItems[id].itemId);
    else
...
{
    CompactPCItems();
    SetItemListPerPageCount(gSaveBlock3Ptr->pcItems, PC_ITEMS_COUNT, &gPlayerPCItemPageInfo.pageItems, &gPlayerPCItemPageInfo.count, 8);
}
...
static void ItemStorage_StartItemSwap(u8 taskId)
{
    ...
    CopyItemName(gSaveBlock3Ptr->pcItems[sItemStorageMenu->toSwapPos].itemId, gStringVar1);
...
static void ItemStorage_FinishItemSwap(u8 taskId, bool8 canceled)
{
    ...
    {
        MoveItemSlotInList(gSaveBlock3Ptr->pcItems, sItemStorageMenu->toSwapPos, newPos);
        ItemStorage_RefreshListMenu();
    }
...
static void ItemStorage_DoItemAction(u8 taskId)
{
    ...
    if (!tInTossMenu)
    {
        if (gSaveBlock3Ptr->pcItems[pos].quantity == 1)
        {
        ...
        end = CopyItemNameHandlePlural(gSaveBlock3Ptr->pcItems[pos].itemId, gStringVar1, 2);
...
static void ItemStorage_HandleQuantityRolling(u8 taskId)
{
    ...
    if (AdjustQuantityAccordingToDPadInput(&tQuantity, gSaveBlock3Ptr->pcItems[pos].quantity) == TRUE)
    {
        ItemStorage_PrintItemQuantity(ItemStorage_AddWindow(ITEMPC_WIN_QUANTITY), tQuantity, STR_CONV_MODE_LEADING_ZEROS, 8, 1, 3);
    }
    else
    {
        ...
        else if (JOY_NEW(B_BUTTON))
        {
            ...
            PlaySE(SE_SELECT);
            ItemStorage_RemoveWindow(ITEMPC_WIN_QUANTITY);
            ItemStorage_PrintMessage(ItemStorage_GetMessage(gSaveBlock3Ptr->pcItems[pos].itemId));
...
static void ItemStorage_DoItemWithdraw(u8 taskId)
{
    ...
    if (AddBagItem(gSaveBlock3Ptr->pcItems[pos].itemId, tQuantity) == TRUE)
    {
        // Item withdrawn
        u8 *end = CopyItemNameHandlePlural(gSaveBlock3Ptr->pcItems[pos].itemId, gStringVar1, tQuantity);
        WrapFontIdToFit(gStringVar1, end, FONT_NORMAL, WindowWidthPx(ITEMPC_WIN_MESSAGE) - 6);
...
static void ItemStorage_DoItemToss(u8 taskId)
{
    ...
    if (!ItemId_GetImportance(gSaveBlock3Ptr->pcItems[pos].itemId))
    {
...
{
    ItemStorage_PrintMessage(ItemStorage_GetMessage(gSaveBlock3Ptr->pcItems[gPlayerPCItemPageInfo.itemsAbove + gPlayerPCItemPageInfo.cursorPos].itemId));
    ItemStorage_ReturnToListInput(taskId);
}

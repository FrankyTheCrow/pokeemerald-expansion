...
static void SetPartyMonFieldSelectionActions(struct Pokemon *mons, u8 slotId)
{
   ...
   // If Mon can learn HM02 and action list consists of < 4 moves, add FLY to action list
    if (sPartyMenuInternal->numActions < 5 && CanLearnTeachableMove(GetMonData(mons, MON_DATA_SPECIES_OR_EGG), MOVE_FLY) && CheckBagHasItem(ITEM_HM_FLY, 1)) 
    AppendToList(sPartyMenuInternal->actions, &sPartyMenuInternal->numActions, 5 + MENU_FIELD_MOVES);
    // If Mon can learn HM05 and action list consists of < 4 moves, add FLASH to action list
    if (sPartyMenuInternal->numActions < 5 && CanLearnTeachableMove(GetMonData(mons, MON_DATA_SPECIES_OR_EGG), MOVE_FLASH) && CheckBagHasItem(ITEM_HM_FLASH, 1)) 
